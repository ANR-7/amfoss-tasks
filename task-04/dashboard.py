...
class Dashboard(QWidget):
    ...
    def toggle_column(self, columns):
        cols = columns.split(",")
        for c in cols:
            if c in self.selected_columns:
                self.selected_columns.remove(c)
            else:
    ...
    def execute_search(self):
        ...
        for col in self.selected_columns:
            if col.lower() == "stars":
                cols.extend(["star1", "star2", "star3"])
            else:
                cols.append(col)
        col_string = ", ".join(cols) if cols else "*"

        # Build SQL depending on mode
        mode = self.mode.lower()
        if mode in ("actor", "stars"):
            sql = f"""
                SELECT {col_string} 
                FROM movies 
        ...
        elif mode in ("title", "year", "genre", "rating", "director", "star1", "star2", "star3"):
            sql = f"SELECT {col_string} FROM movies WHERE {mode} LIKE %s"
            params = (f"%{query}%",)

        else:
            self.output_console.append(f"Unknown search mode: {self.mode}")
            return
    ...
    def export_csv(self):
        ...
        with open("exported_results.csv", "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(headers)
            for r in range(rows):
                writer.writerow([self.table.item(r, c).text() if self.table.item(r, c) else "" for c in range(cols)])

        self.output_console.append("Exported to exported_results.csv")
